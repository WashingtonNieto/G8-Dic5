...
public class OrderService {
    ...
    public Optional<Order> getOrder(int id){return orderRepository.getOrder(id); }
        ...
        if(order.getId()!=null){
            ...
            if(!existeOrder.isEmpty()) {
                if (order.getStatus() != null) {
                    existeOrder.get().setStatus(order.getStatus());
                }
                orderRepository.update(existeOrder.get());
                return existeOrder.get();
            }else{
                return order;
            }
        }else {
    ...
    public List<Order> orderSalesManByDate(Integer id){
